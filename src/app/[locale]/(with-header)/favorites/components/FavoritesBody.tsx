...
export default function FavoritesBody() {
  ...
  const getFavoritesFromCookies = (): ProductType[] => {
    try {
      ...
      if (!cookie) {
        console.log("❌ Cookie 'favorites' не найден");
        return [];
      }

      const parsed = JSON.parse(decodeURIComponent(cookie.split("=")[1]));
      console.log("✅ Полученные товары из куков:", parsed);
      return parsed;
    } catch (error) {
  ...
  const fetchProducts = async () => {
    ...
    if (!session?.user?.access_token) {
      try {
        const allFavorites = getFavoritesFromCookies();
        ...
        setProducts(paginatedFavorites);
        setLastPage(Math.ceil(allFavorites.length / 6));
      } catch (error) {
        console.error("Ошибка при получении избранного из куков:", error);
      } finally {
        setIsLoading(false);
      }
      return;
